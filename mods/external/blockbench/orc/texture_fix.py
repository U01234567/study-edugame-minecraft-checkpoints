...
def extract_mesh_cube(
    root: dict,
    buffers: list[bytes],
    mesh_index: int,
) -> tuple[tuple[int, int, int], dict[str, tuple[float, float, float, float]]]:
    primitive = root["meshes"][mesh_index]["primitives"][0]
    ...
    if not (len(unique_x) <= 2 and len(unique_y) <= 2 and len(unique_z) <= 2):
        raise ValueError(f"Mesh {mesh_index} is not an exact cuboid.")
...
def build_model_png() -> None:
    ...
    # For this creature, ./textures/gltf_embedded_0.png is already the correct
    # sidecar orientation for the crop logic below. Do not flip it here.
    source_image = Image.open(SOURCE_TEXTURE_PATH).convert("RGBA")
    ...
    for mesh_index in range(len(root.get("meshes", []))):
        (dx, dy, dz), source_faces = extract_mesh_cube(root, buffers, mesh_index)
        ...
        if shelf_v + tile_h > ATLAS_HEIGHT:
            raise ValueError("Packed texture exceeds the fixed 128x128 atlas used by model.java.")

        layout = face_layout(shelf_u, shelf_v, dx, dy, dz)
        for face_name, (target_u, target_v, target_w, target_h) in layout.items():
            patch = crop_face(source_image, source_faces[face_name])
