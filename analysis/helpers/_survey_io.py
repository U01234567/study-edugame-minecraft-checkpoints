...
def normalise_headers(raw_header: list[str], labels: list[str], import_row: list[str]) -> list[str]:
    ...
    for index, raw_name in enumerate(raw_header):
        ...
        if import_id:
            chosen = name if name and not name.startswith("QID") and not import_id.startswith("QID") else import_id
        elif name.startswith("QID") and " - " in label:
            chosen = clean(label.split(" - ", 1)[0]) or name
        else:
            chosen = name or f"column_{index + 1}"
        count = seen.get(chosen, 0)
        seen[chosen] = count + 1
        normalised.append(chosen if count == 0 else f"{chosen}_{count + 1}")
    return normalised
